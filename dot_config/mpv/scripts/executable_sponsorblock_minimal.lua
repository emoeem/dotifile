...
local function skip_ads(_, pos)
    if pos ~= nil and ranges ~= nil then
        for _, v in pairs(ranges) do
            if v.segment and v.actionType == "skip" and v.segment[1] <= pos and v.segment[2] > pos then
            ...
            elseif v.segment and v.actionType == "mute" then
...
local function file_loaded()
    ...
    -- 仅对网络视频启用 SponsorBlock，跳过本地文件
    if not video_path:find("://") then return end
...
local function toggle()
    ON = not ON
    mp.osd_message('SponsorBlock: ' .. (ON and '已启用' or '已禁用'))
end

mp.register_event("file-loaded", file_loaded)
mp.register_event("end-file", end_file)
mp.register_script_message("toggle-sponsorblock", toggle)
